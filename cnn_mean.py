...
def load_new_data():
    with (open("mean_subject_sorted/subject_0.pkl", "rb")) as data:
        data = pickle.load(data)
        img = []
        complexity = []
        # separate images from labels
    ...
    with open('test_mean_sorted.pkl', 'wb') as f:
        pickle.dump(test_dataset, f)
    train_dataset = list(zip(X_train, y_train))
    with open('train_mean_sorted.pkl', 'wb') as f:
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, scheduler, epochs, fold=0):
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
    torch.save(model.state_dict(), "newResults/mean/3d_image_classification"+str(fold)+".pth")
    torch.save({
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss': loss},
        "newResults/mean/model"+str(fold)+".pth.tar")
...
def k_fold(k_folds=5, batch_size=batch_size):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(images, labels)):
        ...
        plt.legend()
        plt.ylim(0, 1.2)
        plt.savefig("newResults/mean/TrainingValidationLoss"+ str(fold))
        plt.show()
    ...
    plt.savefig("newResults/mean/F1Scores")
    plt.show()
    results = {"Accuracies": accuracies, "TrainingLosses": losses, "F1Scores": f1_scores}
    with open('newResults/mean/results.pkl', 'wb') as f:
